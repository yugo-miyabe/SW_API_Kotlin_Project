...
class VehiclesViewModel(private val apiRepository: APIRepository) : BaseViewModel() {
...
class VehiclesViewModelFactory(private val apiRepository: APIRepository) :
    ViewModelProvider.Factory {
    override fun <T : ViewModel> create(modelClass: Class<T>): T {
        if (modelClass.isAssignableFrom(VehiclesViewModel::class.java)) {
            @Suppress("UNCHECKED_CAST")
            return VehiclesViewModel(apiRepository) as T
        }
